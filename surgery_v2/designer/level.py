        ...
        for bp, f_ in zip(self.body_parts, self.patches_files):
            ...
            if bp.wound_files:
                draw.rounded_rectangle([(int(name[2]), int(name[3])),
                                       ...
                                       13, None, outline=(88, 95, 52, 255), width=3)
        return canvas
...
class BodyPart:
    ...
    color_map = {
        9: np.array([255, 255, 255], dtype=np.uint8),
        8: np.array([0, 0, 0], dtype=np.uint8),
        7: np.array([63, 81, 181], dtype=np.uint8),
        6: np.array([3, 169, 244], dtype=np.uint8),
    ...
    def canvas_mask_to_ndarray(self):
        mask_nd = np.zeros([*self.m_canvas_np.shape, 10], dtype=np.uint8)
        for key in self.color_map:
    ...
    def _find_places_for_wound(self, f_wound):
        ...
        for iteration in range(1000):
            ...
            img = w_img.copy().rotate(angle, expand=True)

            if os.path.split(f_wound)[1] in ["big.png", "infected.png", "scar.png"]:
                # Scale is allowed
